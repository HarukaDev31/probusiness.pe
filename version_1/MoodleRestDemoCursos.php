...
function get_enrolled_users( $course_id, $token ) 
{
  ...
  $user = xmlresponse_to_all( $response );
  return $user;
}
...
function get_all_course( $params, $token ) 
{
  /*
echo "<pre>";
var_dump($params);
...
*/
  ...
  return xmlresponse_to_user_all( $response );
...
function xmlresponse_parse_names_and_values_all( $xml_string )
{
  ...
  echo "<pre>";
  var_dump($xml_tree->MULTIPLE);
  echo "</pre>";


  foreach ( $xml_tree->MULTIPLE->SINGLE[1]->KEY as $key ) {
    /*
    echo "<pre>";
    var_dump($key);
    echo "</pre>";
...
function getUser($arrPost){
  try {
    $token = '2a41772b01afcf26da875fc1ab59bf45';

    /*
    $user_id_1 = get_user_field( $arrPost, $token );
    return $user_id_1;
    */
    $id='';
    return get_all_course($id, $token);
  } 
  catch ( Exception $e ) {
    return array(
      'status' => 'error',
...
$arrPost['criteria'][0]['key']='username';
$arrPost['criteria'][0]['value']='fabadfernandez';
//$arrPost['criteria'][0]['value']='1111';

$response = getUser($arrPost);
